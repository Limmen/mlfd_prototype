...
class WorkerSpec() extends TestKit(ActorSystem("WorkerSpec"))
  ...
  override def afterAll = {
    ...
    "A Worker with 0% crash" must {
      ...
      val worker = TestActorRef(new Worker(id=1, geoLoc=1, stdDev=1, geoFactor=0, crashProb=0, collector = self, bandwidth=1, bandwidthFactor=0, messageLossProb=0, pattern=true, 0, 1, false, 1, 1))
      worker ! HeartBeat
